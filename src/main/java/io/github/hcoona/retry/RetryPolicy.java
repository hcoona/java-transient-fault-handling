...
public class RetryPolicy {
  ...
  static {
    try {
      ...
      defaultFixed = new RetryPolicy(new TransientErrorCatchAllStrategy(),
          RetryStrategy.getDefaultFixed());
      defaultProgressive = new RetryPolicy(new TransientErrorCatchAllStrategy(),
          RetryStrategy.getDefaultProgressive());
      defaultExponential = new RetryPolicy(new TransientErrorCatchAllStrategy(),
          RetryStrategy.getDefaultExponential());
    } catch (Exception e) {
   ...
   */
  public RetryPolicy(ITransientErrorDetectionStrategy errorDetectionStrategy,
      int retryCount, Duration retryInterval) {
   ...
   */
  public CompletableFuture executeActionAsync(Runnable command,
    ...
    return new AsyncExecution<Void>(callable, executor,
      getRetryStrategy().getShouldRetry(),
      getErrorDetectionStrategy(),
      getRetryStrategy().getFastFirstRetry()).executeAsync();
  }
   ...
   *     will transition to a faulted state and the exception must be observed.
   */
  public <T> CompletableFuture<T> executeActionAsync(Callable<T> command,
      ScheduledExecutorService executor) {
    Preconditions.checkNotNull(command);

    return new AsyncExecution<T>(command, executor,
        getRetryStrategy().getShouldRetry(),
        getErrorDetectionStrategy(),
        getRetryStrategy().getFastFirstRetry()).executeAsync();
  }
